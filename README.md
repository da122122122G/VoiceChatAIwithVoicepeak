...
- Geminiのストリームを文末ごとに分割し、生成途中からVOICEPEAKで合成
...
Windowsから`Terminate batch job (Y/N)?`または`バッチ ジョブを終了しますか (Y/N)?`と確認された場合は、音声チャットを終了するなら`Y`を入力してEnterを押します。誤って`Ctrl+C`を押し、処理を続けたい場合は`N`を選びます。
...
Geminiの返答はストリーミングで受信します。`。`、`！`、`？`などで最初の文が完成した時点でVOICEPEAKのキューへ送り、2文目以降の生成と先頭文の音声合成を並行します。全文の生成完了を待たずに最初の読み上げを開始できます。
...
PythonからVOICEPEAKを自動起動した場合は、プロセス検出だけでなくメインウィンドウの生成と内部UIの初期化も待ちます。それでもBridgeが`READY`前に終了した場合は、3秒、5秒、8秒の間隔で自動再試行します。
